...
description: "MCP 規格從 2024-11-05 的「HTTP with SSE」雙端點機制，演進到 2025-03-26 的「Streamable HTTP」單一端點架構——比較兩版傳輸層的連接流程、雙向通信與遷移影響。"
tags:
  - mcp
  - mcp-server
  - mcp-client
...
- **Session ID 是可選的**：在 Streamable HTTP 規範中，服務器**可以**（MAY）在初始化時分配 Session ID，但這不是必需的。規範明確使用「MAY」而非「MUST」，表明這是一個選擇性而非強製性的。
